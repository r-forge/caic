`macrocaic` <-
function(formula, data, phy, names.col, macroMethod = "RRD", stand.contr = TRUE, ref.var=NULL, node.depth=NULL,
                  macroMinSize=3, equal.branch.length=FALSE)
{
        ...
        # TODO - RETURN CLADE SIZES ON EITHER SIDE OF MACRO CONTRASTS?
    ...
    # GET THE BASIC MODEL MATRICES
        ...
        if(is.empty.model(formula)) stop("Macrocaic analyses require an explanatory variable to determine the direction of contrasts. Models of the form nSpp ~ 1 are not permitted.")
    ...
    # GET RESPONSE MATRIX
        ...
        rownames(macroMf) <- 1:length(macroMf[,1])
        #  - need a vector of reference values: this means looking up the nodalVal 
        #    rows on the analysis tree and placing them on the intermediate tree
        
        #values from analysis tree with labels
        macroRV <- contr$nodVal[,ref.var]
        
        macroContr <- contrCalc(macroMf, analysisPhy, macroRV, macroMethod)
        mrC <- macroContr$contr
        mdC <- contr$contr
    ...
    # FEED THE RETURNED DESIGN AND RESPONSE MATRICES INTO THE MODELLING FUNCTIONS
        ...
        ContrObj$nodalVals$response <- macroContr$nodVal[as.numeric(rownames(macroContr$nodVal)) >= root,,drop=FALSE]
        ContrObj$nodalVals$explanatory <- contr$nodVal[as.numeric(rownames(contr$nodVal)) >= root,,drop=FALSE]                       
        ...
        intNodeSizes <- with(ContrObj$nodalVal, response[as.numeric(rownames(response)) >= root,])
        validNodes[which(intNodeSizes < macroMinSize)] <- FALSE
